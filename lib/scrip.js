...
class Scrip {
  ...
  recordBonusTradeQty(currTradeDate = moment()) {
    ...
    if (currentBonusAction.recordDate.diff(currTradeDate, "days") <= 0) {
      const [ratio_denom, ratio_num] = currentBonusAction.ratio.split(":");
      currentBonusAction["quantity"] = Math.floor(this.balance * ratio_num / ratio_denom);
    }
  }
  ...
  sortBonusActions() {
    ...
    const bonusActions = this.corpActions[Constants.getBonusCode()];
    bonusActions.sort((a,b) => a.recordDate.diff(b.recordDate));
  }
  ...
  sortSplitActions() {
    ...
    splitActions.sort((a,b) => a.execDate.diff(b.execDate));
  }
  ...
  calcCapitalGains() {
    ...
    for (let sellTrade of this.sellTrades) {
      while (sellTrade.quantity > 0 && buyTradeIndex < this.buyTrades.length) {
        ...
        const buyTrade = sameDayBuyTrade && sameDayBuyTrade.price > 0 ? sameDayBuyTrade : this.buyTrades[buyTradeIndex];
        ...
        if (sellTrade.date.diff(periodStartDate) < 0) {
          continue;
        }
